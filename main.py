...
APP_URL = f'https://bibasosus.pythonanywhere.com/{token}'
app = Flask(__name__)
bot.remove_webhook()
bot.set_webhook(url=APP_URL, allowed_updates=['message',  'callback_query', 'chat_member'])

cursor = create_engine("sqlite:///toxic.db", pool_recycle=280)
# ☣️
def get_pil(fid):
...
def handle_text(message, txt):
        ...
        if message.from_user.id == KIRYA:
            return
        res = pers.analyze(text = txt, attributes = [Attributes.TOXICITY], language = "ru")
        if (res["TOXICITY"] > 60) and (message.from_user.id > 0):
